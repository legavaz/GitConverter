...
Процедура ВыполнитьКонвертацию(Знач Хранилище = Неопределено) Экспорт
	...
	Если Параметры.ПоследняяВерсия = 0 Тогда
		...
		Если ЗначениеЗаполнено(Кластер) Тогда
			СервернаяБаза = Справочники.ВременныеБазы.НайтиСвободную(Кластер);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СервернаяБаза) Тогда
			...
			Справочники.ВременныеБазы.Занять(СервернаяБаза);
		...
		Если ЗначениеЗаполнено(СервернаяБаза) Тогда
			Справочники.ВременныеБазы.Освободить(СервернаяБаза);
			Попытка
	...
	Если РеквизитыВерсии.Состояние = Перечисления.СостоянияВерсии.ВерсияПомещена
		...
		Если ЗначениеЗаполнено(РеквизитыВерсии.ВременнаяБаза) Тогда
			...
			Попытка
				...
				Справочники.ВременныеБазы.Освободить(РеквизитыВерсии.ВременнаяБаза);
				Объект.ВременнаяБаза = Справочники.ВременныеБазы.ПустаяСсылка();
...
Процедура ПроверитьПолученныеВерсии(Источник) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Объект.ВременнаяБаза) Тогда
			Попытка
				Справочники.ВременныеБазы.Освободить(Объект.ВременнаяБаза);
			Исключение
			КонецПопытки;
		КонецЕсли;
...
Функция СтрокаСозданияСервернойБазы(Параметры)
	...
	Если Параметры.ТипСУБД = Перечисления.ТипыСУБД.PostgreSQL Тогда
		ИмяСУБД = "PostgreSQL";
	Иначе
		ИмяСУБД = "MSSQL";
	КонецЕсли;
...
// 	Хранилище - СправочникСсылка.ХранилищаКонфигураций - ссылка на хранилище
// Возвращаемое значение:
// 	СправочникСсылка.Кластеры, Неопределено - кластер временных баз, либо Неопределено при файловом режиме
...
// 
// Параметры:
// 	Кластер - СправочникСсылка.Кластеры - кластер временных баз
...
Процедура ПолучитьВерсиюВБазу(Параметры, Хранилище, ВерсияХранилища)
	...
	Если ЗначениеЗаполнено(СервернаяБаза) Тогда
		...
		Справочники.ВременныеБазы.Занять(СервернаяБаза, ВерсияХранилища);
