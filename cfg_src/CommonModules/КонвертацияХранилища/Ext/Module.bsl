	...
	Если Параметры.ПоследняяВерсия = 0 Тогда
		...
		Если ЗначениеЗаполнено(Кластер) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СервернаяБаза) Тогда
				ТекстПредупреждения = "Свободная временная база не обнаружена. Будет использован файловый режим.";
		...
		Если ЗначениеЗаполнено(СервернаяБаза) Тогда
			...
			РеквизитыБазы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СервернаяБаза, "ИмяБазыНаСервере");
...
Функция ПолучитьПараметрыОбработкиВерсии(Знач ВерсияХранилища) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыВерсии.ВременнаяБаза) Тогда
		...
		Если ЗначениеЗаполнено(Кластер) Тогда
			...
			Если ЗначениеЗаполнено(Параметры.ИмяКластера) Тогда
				Сервер = Параметры.АдресСервера + "\\" + Параметры.ИмяКластера;
			КонецЕсли;

			СтрокаСоединения = " /S""%Сервер%"";""%ИмяБазы%""";
			СтрокаСоединения = СтрЗаменить(СтрокаСоединения, "%Сервер%", Сервер);
...
// Если у кластера задан пользователь кластера - возвращается он, иначе пользователь СУБД.
// 
// Параметры:
// 	Кластер - СправочникСсылка.ноКластеры - кластер временных баз
// Возвращаемое значение:
...
Функция ПолучитьИмяПользователяИБ(Кластер)
	...
	ИменаРеквизитов = Новый Структура("ИмяПользователяКластера, ИмяПользователяСУБД");
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Кластер, ИменаРеквизитов);
	...
	Возврат Реквизиты.ИмяПользователяСУБД;
