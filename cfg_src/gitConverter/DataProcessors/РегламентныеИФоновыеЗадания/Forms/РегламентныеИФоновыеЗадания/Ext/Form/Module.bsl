...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	Если НЕ ЗавершениеРаботы И ИдентификаторЗадания <> Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000") Тогда
...
Процедура ОткрытьРасписаниеЗавершение(НовоеРасписание, ТекущиеДанные) Экспорт
	...
	Если НовоеРасписание <> Неопределено Тогда
		Для каждого ВыделеннаяСтрока Из Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки Цикл
			...
			ТекущаяСтрока = ТаблицаРегламентныеЗадания.НайтиПоИдентификатору(ВыделеннаяСтрока);
			УстановитьРасписание(ТекущаяСтрока.Идентификатор, НовоеРасписание);
...
Процедура ОтменитьФоновоеЗаданиеНаСервере(Знач Идентификатор)
	...
	ОбновитьТаблицуРегламентныхЗаданий();
	ОбновитьТаблицуФоновыхЗаданий(Ложь);
...
Процедура ОбновитьТаблицуРегламентныхЗаданий(ИдентификаторРегламентногоЗадания = Неопределено)
	...
	Для Каждого ОбъектМетаданных Из Метаданные.РегламентныеЗадания Цикл
		Если Не РегламентныеЗаданияСлужебный.РегламентноеЗаданиеДоступноПоФункциональнымОпциям(ОбъектМетаданных, ЗависимостиЗаданий) Тогда
...
Процедура ОбновитьТаблицуФоновыхЗаданийНаКлиенте()
	...
	Результат = ФормированиеТаблицыФоновыхЗаданийВФоне();
...
&НаСервере
Функция ФормированиеТаблицыФоновыхЗаданийВФоне()
	
	Отбор = ОтборФоновыхЗаданий();
	ПередаваемыеПараметры = Новый Структура;
	ПередаваемыеПараметры.Вставить("Отбор", Отбор);
	
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФонеСлужебный(
			УникальныйИдентификатор,
			"РегламентныеЗаданияСлужебный.ТаблицаСвойствФоновыхЗаданийВФоне",
			ПередаваемыеПараметры,
			НСтр("ru = 'Регламентные задания. Обновление списка фоновых заданий'")
			, , Ложь);
...
&НаСервере
Процедура ОбновитьТаблицуФоновыхЗаданий(ФоноваяЗагрузка = Истина)
	
	// 2. Обновление списка фоновых заданий.
	
	Если ФоноваяЗагрузка Тогда
