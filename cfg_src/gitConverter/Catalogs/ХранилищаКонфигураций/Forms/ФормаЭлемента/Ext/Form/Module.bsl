...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ИспользоватьСерверныеВременныеБазы И ЗначениеЗаполнено(Объект.КластерВременныхБаз) Тогда
		...
		ЗапросВременныеБазы.УстановитьПараметр("Кластер", Объект.КластерВременныхБаз);
		ЗапросВременныеБазы.УстановитьПараметр("Хранилище", Объект.Ссылка);
		
		ВыборкаВременныеБазы = ЗапросВременныеБазы.Выполнить().Выбрать();
		Если ВыборкаВременныеБазы.Следующий()
				И Объект.МаксимальноеКоличествоПодготавливаемыхВерсий > ВыборкаВременныеБазы.Количество Тогда
			...
			Сообщение.Текст = СтрШаблон(ТекстСообщения,
				Формат(Объект.МаксимальноеКоличествоПодготавливаемыхВерсий, "ЧГ=0"),
				Формат(ВыборкаВременныеБазы.Количество, "ЧГ=0"));
			Сообщение.ПутьКДанным = "Объект.МаксимальноеКоличествоПодготавливаемыхВерсий";
			Сообщение.КлючДанных = Объект.Ссылка;
			Сообщение.Сообщить();
			Отказ = Истина;
...
Процедура ЗапуститьКонвертацию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Хранилище не сохранено. Сохраните элемент и повторите.'"));
		Возврат;
	КонецЕсли;
	
	Проверка = ПроверитьСвободныеВременныеБазыНаСервере();
	Если Проверка.ИспользоватьСерверныеВременныеБазы И НЕ Проверка.ЕстьСвободные Тогда
		ТекстПредупреждения = НСтр("ru = 'В кластере ""%Кластер%"" нет свободных временных баз. Конвертация будет выполнена в файловом режиме.'");
		ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%Кластер%", Строка(Проверка.Кластер));
...
&НаКлиенте
Процедура ctmВыполнитьСинхронизациюГитсинк(Команда)
	...
	Запущено = ctmВыполнитьСинхронизациюГитсинкНаСервере();
	Если НЕ Запущено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Синхронизация gitsync для этого хранилища уже выполняется.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура ОсвободитьВременныеБазы(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОсвободитьВременныеБазыЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Освободить все временные базы, занятые версиями этого хранилища?'"), РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ОсвободитьВременныеБазыЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	Результат = ОсвободитьВременныеБазыНаСервере();
	
	Текст = НСтр("ru = 'Освобождено временных баз: %1. Требуют пересоздания: %2.'");
	Текст = СтрЗаменить(Текст, "%1", Результат.Освобождено);
	Текст = СтрЗаменить(Текст, "%2", Результат.ТребуютПересоздания);
	ПоказатьПредупреждение(, Текст);
...
&НаКлиенте
Процедура ОбнулитьСчетчикВременныхБаз(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбнулитьСчетчикВременныхБазЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Обнулить счетчик использования всех временных баз этого хранилища?'"), РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбнулитьСчетчикВременныхБазЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	Количество = ОбнулитьСчетчикВременныхБазНаСервере();
	
	Текст = НСтр("ru = 'Обнулен счетчик использования временных баз: %1.'");
	Текст = СтрЗаменить(Текст, "%1", Количество);
	ПоказатьПредупреждение(, Текст);
...
&НаСервере
Функция ctmВыполнитьСинхронизациюГитсинкНаСервере()
	
	Ключ = Строка(Объект.Ссылка.УникальныйИдентификатор()) + "_gitsync";
	
	Отбор = Новый Структура("Ключ, Состояние, ИмяМетода",
		Ключ, СостояниеФоновогоЗадания.Активно, "КонвертацияХранилища.ctmВыполнитьСинхронизациюГитсинк");
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор).Количество() > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	НаименованиеЗадания = Лев(НСтр("ru = 'Синхронизация gitsync'") + ": " + СокрЛП(Объект.Адрес), 120);
	
	ФоновыеЗадания.Выполнить("КонвертацияХранилища.ctmВыполнитьСинхронизациюГитсинк", ПараметрыЗадания, Ключ, НаименованиеЗадания);
...
&НаСервере
Функция ПроверитьСвободныеВременныеБазыНаСервере()
	
	Результат = Новый Структура("ИспользоватьСерверныеВременныеБазы, ЕстьСвободные, Кластер");
	Результат.ИспользоватьСерверныеВременныеБазы = Ложь;
	Результат.ЕстьСвободные = Ложь;
	Результат.Кластер = Неопределено;
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Кластер = КонвертацияХранилища.ПолучитьКластерВременныхБаз(Объект.Ссылка);
	Если ЗначениеЗаполнено(Кластер) Тогда
		Результат.ИспользоватьСерверныеВременныеБазы = Истина;
		Результат.Кластер = Кластер;
		Результат.ЕстьСвободные = ЗначениеЗаполнено(Справочники.ноВременныеБазы.НайтиСвободную(Кластер, Объект.Ссылка));
	КонецЕсли;
...
&НаСервере
Функция ОбнулитьСчетчикВременныхБазНаСервере()
	...
	|	И ВременныеБазы.КоличествоИспользований <> 0";
	Запрос.УстановитьПараметр("Хранилище", Объект.Ссылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбъектБазы = Выборка.ВременнаяБаза.ПолучитьОбъект();
		Если ОбъектБазы <> Неопределено Тогда
			ОбъектБазы.КоличествоИспользований = 0;
			ОбъектБазы.Записать();
...
&НаСервере
Функция ОсвободитьВременныеБазыНаСервере()
	
	Результат = Новый Структура("Освобождено, ТребуютПересоздания");
	Результат.Освобождено = 0;
	Результат.ТребуютПересоздания = 0;
	
	// Источник истины - состояния самих баз: база может быть занята без привязки к версии
	// (например, при прерванной конвертации), тогда поиск по версиям ее не находит.
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВременныеБазы.Ссылка КАК ВременнаяБаза
	...
	|	И ВременныеБазы.Состояние В (
	|		ЗНАЧЕНИЕ(Перечисление.ноСостоянияВременныхБаз.Занята),
	|		ЗНАЧЕНИЕ(Перечисление.ноСостоянияВременныхБаз.ТребуетПересоздания))";
	Запрос.УстановитьПараметр("Хранилище", Объект.Ссылка);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		РеквизитыБазы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Выборка.ВременнаяБаза, "Состояние");
		Если РеквизитыБазы.Состояние = Перечисления.ноСостоянияВременныхБаз.ТребуетПересоздания Тогда
			Результат.ТребуютПересоздания = Результат.ТребуютПересоздания + 1;
			Продолжить;
		КонецЕсли;
		
		Справочники.ноВременныеБазы.Освободить(Выборка.ВременнаяБаза);
		Результат.Освобождено = Результат.Освобождено + 1;
		
		ОчиститьВерсииСВременнойБазой(Выборка.ВременнаяБаза);
...
&НаСервере
Процедура ОчиститьВерсииСВременнойБазой(Знач ВременнаяБаза)
	...
	|	Справочник.ВерсииХранилища КАК ВерсииХранилища
	|ГДЕ
	|	ВерсииХранилища.ВременнаяБаза = &ВременнаяБаза";
